...
// Fitur Hapus (Opsional, jika ingin digabung di sini)
if (isset($_GET['hapus'])) {
...
if (isset($_POST['simpan'])) {
    ...
    // Gunakan mysqli_real_escape_string untuk semua input teks
    ...
    // Gunakan sql_val untuk data tanggal/data yang bisa kosong
    ...
    if(isset($_POST['nm_pas'])) {
        foreach($_POST['nm_pas'] as $k => $v) {
            if($v) {
                ...
                mysqli_query($conn, "INSERT INTO pasangan (pegawai_id, nama_pasangan, tanggal_lahir, tanggal_perkawinan, pekerjaan, penghasilan_sebulan, keterangan) 
                             VALUES ($id, '$v_esc', $lhr, $nik, '$job', '$inc', '$ket')");
            }
        }
    }
    ...
    if(isset($_POST['nm_anak'])) {
        foreach($_POST['nm_anak'] as $k => $v) {
            if($v) {
                ...
                $q_anak = "INSERT INTO anak (pegawai_id, nama_anak, status_anak, tanggal_lahir, sekolah_kuliah_pada, masuk_daftar_gaji, nama_ayah, nama_ibu, tgl_wafat_cerai_ortu, status_bekerja, keterangan, status_belum_kawin, tidak_dapat_beasiswa, tidak_dapat_ikatan_dinas) 
                           VALUES ($id, '$v_esc', '$st', $lhr, '$sek', '$msk', '$ayah', '$ibu', $ortu, $kerja, '$ket', 'Ya', '$bea', '$dinas')";
